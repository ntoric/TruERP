...
/// Resolve resource roots used to find bundled Node, Next server, and API binary.
...
pub fn find_api_binary(roots: &[PathBuf]) -> Option<PathBuf> {
    #[cfg(windows)]
    let names = ["bin/truerp-api.exe", "truerp-api.exe", "bin/truerp-api"];
    #[cfg(not(windows))]
    let names = ["bin/truerp-api", "truerp-api"];
    ...
    // Dev fallback: run `go run` against ../backend via a wrapper script is not used;
    // instead look for a freshly built binary next to the repo backend.
    if let Ok(cwd) = std::env::current_dir() {
        let candidates = [
            cwd.join("../backend/truerp"),
            cwd.join("../backend/truerp-api"),
            cwd.join("../../backend/truerp"),
            cwd.join("resources/bin/truerp-api"),
        ];
        for c in candidates {
            if c.exists() {
                return Some(c);
