...
mkdir -p "${RES_DIR}/runtime" "${RES_DIR}/bin" "${RES_DIR}/node"

echo "==> AI models are not packaged (by design)"

if [[ -f "${REPO_ROOT}/HSN_DATASET.csv" ]]; then
  echo "==> Copying HSN_DATASET.csv"
  cp "${REPO_ROOT}/HSN_DATASET.csv" "${RES_DIR}/HSN_DATASET.csv"
  cp "${REPO_ROOT}/HSN_DATASET.csv" "${RES_DIR}/runtime/HSN_DATASET.csv"
else
...
if [[ ! -f "${RES_DIR}/bin/truerp-api" && ! -f "${RES_DIR}/bin/truerp-api.exe" ]]; then
  echo "==> API binary missing — running prepare-api.sh"
  "${ROOT_DIR}/scripts/prepare-api.sh"
fi
...
if [[ ! -f "${RES_DIR}/server/server.js" ]]; then
  if [[ -f "${REPO_ROOT}/desktop/frontend/server/server.js" ]]; then
    ...
    echo "warning: frontend server missing — run ./scripts/prepare-frontend.sh"
  fi
fi
