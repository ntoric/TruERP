...
import { 
  ...
  FileText, Printer, Users, Bell, HelpCircle, Share2, LogOut, Code, Scale 
} from 'lucide-react'
                ...
                <form onSubmit={handleCreateUser} className="space-y-4">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input id="user_name" value={newUser.name} onChange={(e) => handleChange('name', e.target.value, setNewUser)} required />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="user_email">Email *</Label>
                      <Input id="user_email" type="email" value={newUser.email} onChange={(e) => handleChange('email', e.target.value, setNewUser)} required />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="user_password">Password *</Label>
                      <Input id="user_password" type="password" value={newUser.password} onChange={(e) => handleChange('password', e.target.value, setNewUser)} required minLength={6} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="user_phone">Phone</Label>
                      <Input id="user_phone" value={newUser.phone} onChange={(e) => handleChange('phone', e.target.value, setNewUser)} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="user_role">Role *</Label>
                      <Select value={newUser.role} onValueChange={(value) => handleChange('role', value, setNewUser)}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          checked={business.enable_ai_hsn_search}
                          onCheckedChange={(checked) => handleChange('enable_ai_hsn_search', checked, setBusiness)}
                        />
                        <Label htmlFor="enable_ai_hsn_search">Enable AI-powered HSN code search using Gemini</Label>
                      </div>
                      <p className="text-xs text-gray-500">When enabled, you can use AI to find HSN codes based on product descriptions</p>
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id="enable_ai_bill_parsing"
                          checked={business.enable_ai_bill_parsing}
                          onCheckedChange={(checked) => handleChange('enable_ai_bill_parsing', checked, setBusiness)}
                        />
                        <Label htmlFor="enable_ai_bill_parsing">Enable AI-powered purchase bill parsing using Gemini</Label>
                      </div>
                      <p className="text-xs text-gray-500">When enabled, you can upload bill images and AI will extract vendor details, items, and amounts</p>
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="gemini_api_key"
                        type="password"
                        value={business.gemini_api_key}
                        onChange={(e) => handleChange('gemini_api_key', e.target.value, setBusiness)}
                        placeholder="Enter your Gemini API key"
                        disabled={!business.enable_ai_hsn_search && !business.enable_ai_bill_parsing}
                      />
                      <p className="text-xs text-gray-500">Get your API key from <a href="https://makersuite.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Google AI Studio</a></p>
                    </div>
                  </div>
