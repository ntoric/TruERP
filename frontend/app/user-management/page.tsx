...
function roleLabel(role: string) {
  if (role === 'owner' || role === 'super_admin') return 'Super Admin'
...
function canAccessUserManagement(role: string) {
...
export default function UserManagementPage() {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', password: '', phone: '', role: 'staff' })
  ...
  const superAdmins = useMemo(
    () => users.filter((u) => u.role === 'owner' || u.role === 'super_admin'),
    [users]
  )
  const admins = useMemo(() => users.filter((u) => u.role === 'admin'), [users])
  const staffUsers = useMemo(
    () => users.filter((u) => ['staff', 'accountant', 'manager'].includes(u.role)),
    [users]
  )
  ...
  const fetchCore = useCallback(async () => {
    setLoading(true)
    try {
      const [overviewRes, usersRes, rolesRes, permsRes, activityRes, auditRes, twoFaRes] = await Promise.all([
        apiFetch('/user-management/overview'),
  ...
  useEffect(() => {
    if (!authLoading && user && canAccessUserManagement(user.role)) {
      fetchCore()
    } else if (!authLoading) {
      setLoading(false)
    }
  ...
  const handleUpdateUser = async (u: AppUser, patch: Partial<AppUser>) => {
    setSaving(true)
    try {
  ...
  const handleSaveRole = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = editingRoleId
        ? await apiFetch(`/compliance/roles/${editingRoleId}`, { method: 'PUT', body: JSON.stringify({ ...payload, is_active: true }) })
      ...
      <DashboardLayout>
        <Card className="max-w-lg">
          <CardHeader>
            ...
            <CardDescription>Only Super Admin and Admin roles can manage users and permissions.</CardDescription>
          </CardHeader>
        </Card>
      </DashboardLayout>
    )
      ...
      <TableBody>
        {paginatedItems.map((u) => (
          <TableRow key={u.id}>
            ...
            <TableCell>
              {allowRoleEdit && !['owner', 'super_admin'].includes(u.role) ? (
                <Select
                  value={u.role}
                  onValueChange={(value) => handleUpdateUser(u, { role: value } as Partial<AppUser>)}
                >
            ...
            <TableCell>
              <Switch
                checked={u.is_active}
                disabled={['owner', 'super_admin'].includes(u.role)}
                onCheckedChange={(checked) => handleUpdateUser(u, { is_active: checked } as Partial<AppUser>)}
              />
            </TableCell>
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Super Admin, admin and staff accounts, role permissions, activity logs, audit trails, and 2FA.
          </p>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Card>
                ...
                <CardContent className="text-2xl font-bold">{overview?.total_users ?? 0}</CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium text-muted-foreground">Roles defined</CardTitle>
                </CardHeader>
                <CardContent className="text-2xl font-bold">{overview?.roles_defined ?? 0}</CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium text-muted-foreground">Activity today</CardTitle>
                </CardHeader>
                <CardContent className="text-2xl font-bold">{overview?.activity_today ?? 0}</CardContent>
              </Card>
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium text-muted-foreground">2FA enabled</CardTitle>
                </CardHeader>
                <CardContent className="text-2xl font-bold">{overview?.two_factor_enabled ?? 0}</CardContent>
              </Card>
            </div>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Business owners ({overview?.super_admin_count ?? 0} super admin, {overview?.admin_count ?? 0} admin,{' '}
                  {overview?.staff_count ?? 0} staff).
                </CardDescription>
              </CardHeader>
              <CardContent>
                <UserTable rows={superAdmins} />
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="super-admin">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Full platform access including user management and audit settings.</CardDescription>
              </CardHeader>
              <CardContent>
                <UserTable rows={superAdmins} />
              </CardContent>
          ...
          <TabsContent value="admins">
            <Card>
              ...
              <CardContent>
                <UserTable rows={admins} allowRoleEdit />
                {admins.length === 0 && <p className="text-sm text-muted-foreground">No admin users yet.</p>}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="staff" className="space-y-4">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <UserTable rows={staffUsers} allowRoleEdit />
              </CardContent>
            </Card>
          </TabsContent>
              ...
              <CardContent className="space-y-6">
                <form onSubmit={handleSaveRole} className="space-y-4 rounded-lg border p-4">
                  ...
                  <div className="grid max-h-48 grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2">
                    {permissions.map((p) => (
                      <label key={p.id} className="flex items-start gap-2 text-sm">
                        <Checkbox checked={roleForm.permissionIds.includes(p.id)} onCheckedChange={() => togglePermission(p.id)} />
                        <span>
                  ...
                  <div className="flex gap-2">
                    ...
                    {editingRoleId && (
                      <Button type="button" variant="outline" onClick={() => { setEditingRoleId(null); setRoleForm({ name: '', description: '', permissionIds: [] }) }}>
                        Cancel edit
                      </Button>
                    )}
                  </div>
                ...
                <div className="space-y-3">
                  {roles.map((role) => (
                    <div key={role.id} className="flex items-start justify-between rounded-lg border p-3">
                      <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {!role.is_default && (
                          <Button size="sm" variant="destructive" onClick={() => handleDeleteRole(role.id)}>Delete</Button>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Immutable record of changes ({overview?.audit_total ?? 0} total). Export and retention on the full audit dashboard.
                  </CardDescription>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4 max-w-lg">
                ...
                {!twoFa.enabled && (
                  <>
                    {!twoFa.secret && (
                      <Button onClick={setup2FA}>Start 2FA setup</Button>
                    )}
                    {twoFa.secret && twoFa.otpauthUrl && (
                      <div className="space-y-4 rounded-lg border p-4">
                        <p className="text-sm text-muted-foreground">
                          Add this account in your authenticator app using a QR scan or the manual setup key.
                        </p>
                        <Tabs
                          value={twoFa.setupView}
                          onValueChange={(v) => setTwoFa({ ...twoFa, setupView: v as 'qr' | 'key' })}
                        >
                          ...
                          <TabsContent value="qr" className="space-y-3">
                            <div className="flex justify-center rounded-lg bg-white p-4">
                              <QRCodeSVG value={twoFa.otpauthUrl} size={200} level="M" includeMargin />
                            </div>
                          ...
                          <TabsContent value="key" className="space-y-3">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Choose &quot;Enter a setup key&quot; in your app and paste this value. Time-based (TOTP), 6 digits.
                            </p>
                          </TabsContent>
                        </Tabs>
                      </div>
                ...
                {twoFa.enabled && (
                  <div className="space-y-2">
                    ...
                    <Input type="password" value={twoFa.disablePassword} onChange={(e) => setTwoFa({ ...twoFa, disablePassword: e.target.value })} />
                    <Label>Optional current 2FA code</Label>
                    <Input value={twoFa.code} onChange={(e) => setTwoFa({ ...twoFa, code: e.target.value })} />
