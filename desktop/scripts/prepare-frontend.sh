...
SERVER_OUT="${ROOT_DIR}/frontend/server"
...
# Same-origin API via the Wails asset-server proxy (see desktop/frontend_server.go).
...
echo "==> Publishing standalone server to frontend/server"
rm -rf "${SERVER_OUT}"
mkdir -p "${SERVER_OUT}"
